...
window.onload = function () {
  ...
  var bossCount = 0;
  for (i = 0; i < BossenemyShot.length; i++)
    BossenemyShot[i] = new BossenemyShot();
  ...
  (function () {
    ...
    if (!IsSlow || slowCount % 5 == 0) {
      ...
      // 自機 + 自機のたま
      {
        ...
        if (point < -Infinity) {
          Byoushachara = 10;
          chara.size = Byoushachara / 10 * 4;
          Sabchara = Byoushachara / 2;
        ...
        ChangeColor();
        ctx.beginPath();
        ctx.arc(chara.position.x, chara.position.y, Byoushachara, 0, Math.PI * 2, false);
        ctx.fillStyle = CHARA_COLOR;
        ctx.fill();
        if (IsSlow) {
          ctx.beginPath();
          ctx.arc(chara.position.x, chara.position.y, Byoushachara + 1, 0, Math.PI * 2, false);
        ...
        //外周左右自機        
        ctx.beginPath();　
        ctx.arc(chara.position.x + 30, chara.position.y + 10, Saboutchara + 1, 0, Math.PI * 2, false);
        ctx.moveTo(chara.position.x - 30 + Saboutchara + 1, chara.position.y + 10);
        ctx.arc(chara.position.x - 30, chara.position.y + 10, Saboutchara + 1, 0, Math.PI * 2, false);
